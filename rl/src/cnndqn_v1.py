...
class ConvDQN(nn.Module):
    def __init__(self, num_stack_frames=4, channels_per_frame=8, other_input_dim=8, output_dim=5):
        super(ConvDQN, self).__init__()

        # Input shape: [batch, 4, 8, 7, 5] → stack & reshape to [batch, 32, 7, 5]
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=num_stack_frames * channels_per_frame, out_channels=64, kernel_size=3, padding=1),
            nn.ReLU(),
        ...
        # Compute flattened conv output size (e.g., from 128 channels * 7 * 5)
        dummy_input = torch.zeros(1, num_stack_frames * channels_per_frame, 7, 5)
        conv_output_dim = self.conv(dummy_input).shape[1]

        # MLP after concatenating conv features with direction, location, etc.
        self.fc = nn.Sequential(
            nn.Linear(conv_output_dim + other_input_dim, 256),
            nn.ReLU(),
            nn.Linear(256, 256),
            nn.ReLU(),
    ...
    def forward(self, stacked_viewcone, other_features):
        """
        stacked_viewcone: tensor of shape [batch, 32, 7, 5]
        other_features: tensor of shape [batch, 8]
        """
        x = self.conv(stacked_viewcone)
...
class TrainableRLAgent:
    ...
    def process_observation(self, observation_dict):
        """
        Returns:
            stacked_viewcone_tensor: np.ndarray of shape [32, 7, 5]
            other_features: np.ndarray of shape [8]
        """
        # === Process stacked viewcone ===
        stacked_viewcone = observation_dict.get("stacked_viewcone", None)
        viewcone_frames = []

        if stacked_viewcone is not None:
            frames = stacked_viewcone
        else:
            # If not stacked, use a single frame and pad with zeros
            frames = [observation_dict.get("viewcone", [[0]*5]*7)] + [[[0]*5]*7] * (STACK_SIZE - 1)
        ...
        for frame in frames:
            ...
            for r in range(7):
                for c in range(5):
                    ...
                    tile_features = self._unpack_viewcone_tile(tile_value)  # 8-dim per tile
                    frame_channels.append(tile_features)
            # Shape: [7*5, 8] → [8, 7, 5]
            frame_channels = np.array(frame_channels).reshape(7, 5, 8).transpose(2, 0, 1)  # [C, H, W]
            viewcone_frames.append(frame_channels)

        # Stack into shape: [32, 7, 5]
        stacked_viewcone_tensor = np.concatenate(viewcone_frames, axis=0)
        ...
        other_features.extend(direction_one_hot)
        ...
        other_features.extend([norm_x, norm_y])

        # Scout role
        scout_role = float(observation_dict.get("scout", 0))
        other_features.append(scout_role)

        # Normalized step
        step = observation_dict.get("step", 0)
        norm_step = step / MAX_STEPS_PER_EPISODE if MAX_STEPS_PER_EPISODE > 0 else 0.0
        other_features.append(norm_step)

        # === Return separate tensors ===
        return (
            stacked_viewcone_tensor.astype(np.float32),  # [32, 7, 5]
            np.array(other_features, dtype=np.float32)   # [8]
        )
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        for pet_agent_id in env.agent_iter(): # PettingZoo's iterator
            ...
            if done: # If an agent is done, it might not take an action
                ...
                current_image_tensor, current_flat_tensor = agent.process_observation(obs_dict)
                ...
                if last_observation_for_my_agent is not None:
                    ...
                    (prev_image_tensor, prev_flat_tensor), prev_action, _ = last_observation_for_my_agent
                    agent.step((prev_image_tensor, prev_flat_tensor), prev_action, reward, (current_image_tensor, current_flat_tensor), done)
                    total_steps_taken +=1

                # 3. Select action
                action = agent.select_action((current_image_tensor, current_flat_tensor), epsilon)
                
                # 4. Store current state, action, and this step's reward for the *next* transition
                last_observation_for_my_agent = ((current_image_tensor, current_flat_tensor), action, reward) # reward here is for the current (s,a)
            ...
            if done and pet_agent_id == my_agent_id and last_observation_for_my_agent is not None:
                ...
                (prev_image_tensor, prev_flat_tensor), prev_action, _ = last_observation_for_my_agent
                zero_image_tensor = torch.zeros_like(prev_image_tensor)
                zero_flat_tensor = torch.zeros_like(prev_flat_tensor)
                agent.step((prev_image_tensor, prev_flat_tensor), prev_action, reward, (zero_image_tensor, zero_flat_tensor), True)
                total_steps_taken +=1
...
if __name__ == '__main__':
    ...
    try:
        ...
        trained_scores = train_agent(
            ...
            load_model_from="", # "trained_dqn_agent.pth" to resume
            save_model_to="cnndqn_100k_eps.pth",
            render_mode="rgb_array",
            video_folder="./rl_renders_cnndqn"
        )
